...
class EditPaperTopicForm extends Component {
  constructor(props){
    super(props);
    autobind(this)
    this.state = {
	...
	deleteTopic(index){
		...
		self.setState({topipcs: new_topics, topics_list: new_topics_list}, this.props.setTopics(new_topics_list))
	}
	...
	getPaperTopics(){
		const self = this;
		axios.get('http://localhost:3003/ischolar/get-topic-by-paper', {
			params:{
				paper_id: self.state.paper_id
			}
		})
		.then(function(response){
			self.setState({
				topics_list: self.getTopics(response.data[0]), topics: self.setTopics(response.data[0])
	...
	getTopics(e){
		...
		const new_topics = []
		while(n < e.length){
			new_topics.push(e[n].topic)
			n++
		}
		self.props.setInit(new_topics);
		return new_topics
	}
	...
	setTopics(e){
		...
		const new_topics = []
		while(n < e.length){
			new_topics.push(n+'ID')
			n++
		}
		self.setState({id: new_topics.length}, self.props.setTopics(self.getTopics(e)));
		return new_topics
	}
    ...
    return (
      <div className="inner-right-content">
				...
				{this.state.topics.map((top,index) =>{
					return(<Topic delete={this.deleteTopic} topic={this.state.topics_list[index]} currIndex={index} key={top} changeTopic={this.changeTopic}/>);
				})}
      </div>
    );
  }
...
class Topic extends Component{
	...
	render(){
		return(
		<div>
			  <input type="text" className="topic" value={this.state.topic} placeholder="Topic"  maxLength="20" onChange={this.changeHandler}/>
...
export default EditPaperTopicForm;
